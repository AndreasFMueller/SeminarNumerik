...
\end{equation}
bekannt. 
In der Numerik existiert die Möglichkeit von $\Delta x \rightarrow 0$ nicht, da die Datenpunkte diskretisiert sind. Stattdessen sei $h$ der kleinstmögliche Abstand zweier benachbarter Datenpunkte. Entsprechend kann man die numerische Ableitung (Differenzenquotient) als Quotienten
\begin{equation}
\begin{split}
...
Die Ableitung dieses Polynoms sollte dann eine höhere Genauigkeit aufweisen, da durch die höhere Ordnung die Ursprungsfunktion besser um den Punkt der Ableitung approximiert werden kann.
Als Beispiel soll die erste Ableitung mit fünf Stützstellen in gleichem Abstand $h$ berechnet werden:
\begin{equation}
\frac{\partial f}{\partial x} \approx Af(x-2h) + Bf(x-h) + Cf(x) + Df(x+h) + Ef(x+2h) \text{.}
\label{ableitung:eqn:taylor-support}
\end{equation}
Die Koeffizienten $A, \cdots, E$ sind so zu bestimmen, dass der Ausdruck rechts in möglichst hoher Ordnung in $h$ mit der Ableitung übereinstimmt.
...
\end{split}
\end{equation}
Nach Einsetzen dieser Gleichungen in die Gleichung \ref{ableitung:eqn:taylor-support} ist dürfen nur die Koeffizienten bestehen bleiben und müssen folglich 1 ergeben. Der Rest sollte verschwinden. Zur Unterstützung der Lesbarkeit können die Terme nach Ableitungsgrad sortiert werden:
\begin{equation}
\begin{linsys}{6}
...
%\end{split}
%\end{equation}
Dieses lineare Gleichungssystem für die gesuchten Koeffizienten lässt sich auch in Matrix Form wie folgt schreiben:
\begin{align}
\begin{bmatrix}
1 & 1 & 1 & 1 & 1 \\
-2 & -1 & 0 & 1 & 2 \\
...
\end{equation}
als sehr schwer zu approximieren. Der Fehler $R_n$ wächst ab $x>1$ nach $\infty$.
